...
Схожу логіку я застосовую і в роботі з commits. Husky і lint-staged проганяють виправлення перед commit, а найближчий config обробляє staged files, тож файли в root можуть залишатися на root config, тоді як застосунки й бібліотеки тримають власний локальний setup. При цьому `apps`, `packages` і `libs` ігноруються для root перевірок, щоб форматування, лінтинг і перевірка типів у root були сфокусовані на файлах, які належать йому.
