...
Тут також допомагають conventional commits, у яких особливо корисними є commit scopes. Наприклад, з `feat(api):` або `fix(client):` я ще до відкриття diff бачу, яка саме частина системи змінилася, тоді як звичайний `feat:` зазвичай означає, що зміна зачіпає кілька застосунків або весь репозиторій. Це спрощує і читання історії, і генерацію changelog. У цьому репозиторії передбачувані commit scopes також допомагають скрипту `update:changelog`, який є частиною release workflow в `scripts/release.ts`. Це невелика конвенція, яку підтримують commitlint і husky, але з часом вона добре окупається.
