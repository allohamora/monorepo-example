...
У підсумку цей підхід тримається на кількох простих рішеннях, які добре поєднуються між собою. `Npm` workspaces відповідають за локальне зв'язування пакетів, внутрішні пакети без окремого build прибирають нескінченний цикл rebuilds, сучасний `node.js` спрощує typescript workflow, а `turborepo` лишається лише там, де справді дає виграш.
